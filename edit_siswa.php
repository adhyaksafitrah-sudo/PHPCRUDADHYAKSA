...
// Pastikan beberapa jurusan dan kelas penting ada di database (insert jika belum)
$required_jurusans = [
    'Teknik Pemrograman dan Framework (TPFL)',
    'Teknik Otomasi (TO)',
    'Bisnis Konstruksi dan Properti (BCF)'
];
foreach ($required_jurusans as $rj) {
    $chk = mysqli_query($koneksi, "SELECT id_jurusan FROM jurusan WHERE nama_jurusan = '" . mysqli_real_escape_string($koneksi, $rj) . "'");
    if (!$chk || mysqli_num_rows($chk) == 0) {
        $mx = mysqli_query($koneksi, "SELECT IFNULL(MAX(id_jurusan),0) AS mx FROM jurusan");
        $mr = mysqli_fetch_assoc($mx);
        $next = (int)$mr['mx'] + 1;
        mysqli_query($koneksi, "INSERT INTO jurusan (id_jurusan, nama_jurusan) VALUES ('" . mysqli_real_escape_string($koneksi, $next) . "', '" . mysqli_real_escape_string($koneksi, $rj) . "')");
    }
}

$required_kelas = ['X','XII'];
foreach ($required_kelas as $rk) {
    $chk = mysqli_query($koneksi, "SELECT id_kelas FROM kelas WHERE nama_kelas = '" . mysqli_real_escape_string($koneksi, $rk) . "'");
    if (!$chk || mysqli_num_rows($chk) == 0) {
        $mx = mysqli_query($koneksi, "SELECT IFNULL(MAX(id_kelas),0) AS mx FROM kelas");
        $mr = mysqli_fetch_assoc($mx);
        $next = (int)$mr['mx'] + 1;
        mysqli_query($koneksi, "INSERT INTO kelas (id_kelas, nama_kelas) VALUES ('" . mysqli_real_escape_string($koneksi, $next) . "', '" . mysqli_real_escape_string($koneksi, $rk) . "')");
    }
}
?>
            ...
            <footer class="sticky-footer bg-white">
                <div class="container my-auto">
                    <div class="copyright text-center my-auto">
                        <span>Copyright &copy;Adhyaksa | WebDev School</span>
                    </div>
                </div>
            </footer>
            <!-- End of Footer -->
